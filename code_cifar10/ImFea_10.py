'''
@to do:save hog,hsv,kmeans,conv feature on cifar10
@link:
'''
...
def getEstimator(imgs):
    ...
    for i in range(numPatches):
        ...
        for i in range(3):
            patch = np.append(patch, img[start_r:start_r + rfSize, start_c:start_c + rfSize, i].ravel())
    ...
    [E, V] = np.linalg.eig(np.cov(patches, rowvar=False))  # False/0,每一列代表一个变量sample，而行包含观察值
    ...
    ################use kmeans###################################
    # 构造聚类器
    estimator = MiniBatchKMeans(n_clusters=numCentroids, init='k-means++',max_iter=num_iters, batch_size=1000,n_init=2)
...
def sliding(img,window=[6,6]):
    out=np.array([])
    for i in range(3):
        windows=view_as_windows(img[:,:,i],window,step=1)
        windows=windows.reshape(-1,window[0]*window[1])
        if len(out)==0:
            out=windows
        else:
            out=np.append(out,windows,axis=1)
    return out
...
def saveHsvFea(x_train,x_val,x_test):
    ...
    '''保存所有特征'''
    np.save("../result_cifar10/x_train_hsvs.npy", x_train_hsv)
    np.save("../result_cifar10/x_val_hsvs.npy", x_val_hsv)
    np.save("../result_cifar10/x_test_hsvs.npy", x_test_hsv)
...
def saveHogFea(x_train,x_val,x_test):
    ...
    '''保存所有特征'''
    np.save("../result_cifar10/x_train_hog.npy", x_train_hog)
    np.save("../result_cifar10/x_val_hog.npy", x_val_hog)
    np.save("../result_cifar10/x_test_hog.npy", x_test_hog)
...
def saveKmeansFea(x_train,x_val,x_test):
    ...
    '''保存所有特征'''
    np.save("../result_cifar10/x_train_kmeans.npy", x_train_kmeans)
    np.save("../result_cifar10/x_val_kmeans.npy", x_val_kmeans)
    np.save("../result_cifar10/x_test_kmeans.npy", x_test_kmeans)
...
def saveConvFea(x_train, x_val, x_test, kernel_number_para,pca_para):
    ...
    '''保存所有特征'''
    np.save("../result_cifar10/x_train_conv.npy", x_train_conv)
    np.save("../result_cifar10/x_val_conv.npy", x_val_conv)
    np.save("../result_cifar10/x_test_conv.npy", x_test_conv)
